...
ajax_urls = [
    re_path(r'^(?P<job_id>[\d.@+-]+)/action/delete$', jobs_table_views.delete, name='delete_job'),
    re_path(r'^(?P<job_id>[\d.@+-]+)/action/resubmit$', jobs_table_views.resubmit, name='resubmit_job'),
    re_path(r'^(?P<job_id>[\d.@+-]+)/action/show-log', jobs_table_views.show_log, name='show_log'),
    re_path(r'^(?P<job_id>[\d.@+-]+)/log-content/(?P<key1>[\w+_-]+)(?:/(?P<key2>.*))?',
            jobs_table_views.get_log_content, name='log_content'),
    re_path(r'^(?P<job_id>[\d.@+-]+)/action/update-row', jobs_table_views.update_row, name='update_job_row'),
