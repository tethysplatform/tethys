...
class PlotView(TethysGizmoOptions):
    ...
    Alternatively, developers can specify any valid option from the `Highcharts JavaScript API <http://api.highcharts.com/highcharts>`_ using equivalent Python dictionaries. The recommended strategy for creating custom plots using dictionaries and the Highcharts Plot View is to find a similar example from the `Highcharts demos <http://www.highcharts.com/demo/>`_ page. Use the *Edit in jsFiddle* link to see the source code. Use the JavaScript source of the example to construct the equivalent in Python. Here are a few tips for converting from JavaScript to Python:
    ...
    * Python datetime objects (see the timeseries plot example).
