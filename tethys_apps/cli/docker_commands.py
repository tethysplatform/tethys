        ...
        for block in stream:
            ...
            for line in lines:
                ...
                if current_status not in STATUSES:  # Assume this is the header
                    stdscr.addstr(0, 0, current_status)
                    stdscr.addstr(1, 0, '-' * len(current_status))
                else:
                    if current_status in PROGRESS_STATUSES:
                        ...
                        # add all other messages to list to show above prgress messages
                        messages.append("{id}: {status} {progress}".format(id=current_id, status=current_status,
                                                                           progress=current_progress))
                    ...
                    number_of_rows, number_of_columns = maxy - 1, maxx - 1
                    ...
                    current_messages = messages[-number_of_message_rows:]

                    # print normal messages to proper possion on screen
                    for index, current_message in enumerate(reversed(current_messages)):
                        row = number_of_rows - number_of_progress_rows - index
                        message = current_message + ' ' * (number_of_columns - len(current_message))
                        stdscr.addstr(row, 0, message)

                    # print progress messages to proper position on screen
                    for index, message in enumerate(reversed(current_progress_messages)):
                        row = number_of_rows - index
                        message = "{id}: {status} {progress}".format(**message)
                        stdscr.addstr(row, 0, message)
                    stdscr.refresh()
