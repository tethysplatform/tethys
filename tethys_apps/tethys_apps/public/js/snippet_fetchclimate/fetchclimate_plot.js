...
var FETCHCLIMATE_PLOT = (function() {
    ...
    var charts = jQuery("#fetchclimate_plot .highcharts-plot").each(function() {
      ...
      jQuery.when.apply(jQuery, requests).then(function() {
        ...
        if (data.length>0 && typeof all_data[0] !== 'undefined') {
          ...
          chart_HTML.find('.progress .bar').css('color', 'red');
        }
  ...
  addLoadingAttrToCharts = function() {
    var status = '<div class="job-percentage progress progress-striped active plot-loading">' +
                 '<div class="bar" style="width: 0%;color:black;">Requesting:1/'+
                  m_date_queries.length+'...</div></div>';
    var charts = jQuery("#fetchclimate_plot .highcharts-plot").each(function() {
      var chartHTML = jQuery(this);
      jQuery(status).appendTo(chartHTML);
