...
@mock.patch("tethys_quotas.utilities.log")
@override_settings(RESOURCE_QUOTA_HANDLERS=["tethys_quotas.handlers.workspace.WorkspaceQuotaHandler"])
@pytest.mark.django_db
...
def test_can_add_file_invalid_codename():
    with pytest.raises(ValueError) as context:
        ...
        str(context.exception) == "Invalid codename: invalid_codename"
