...
class TestComponentLibrary(TestCase):
    def test_standard_library_workflow(self):
        ...
        # REGISTER PACKAGE
        lib.register("my-react-package@0.0.0", "does_not_exist", styles=['my_style.css'], use_default=True)
        self.assertIn('does_not_exist', lib.PACKAGE_BY_ACCESSOR)
        self.assertEqual(lib.PACKAGE_BY_ACCESSOR['does_not_exist'], 'my-react-package@0.0.0')
        ...
        self.assertRaises(ValueError, lib.register, 'different-react-package@1.1.1', 'does_not_exist')
        ...
        self.assertDictEqual(lib.components_by_package, {'pigeon-maps@0.21.6': ['Map'], 'react-bootstrap@2.10.2': ['Button'], 'my-react-package@0.0.0': ['Test']})
