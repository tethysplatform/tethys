...
class TestTethysTemplateLoader(unittest.TestCase):
    ...
    def test_get_contents(self, _, mock_file):
        handlers = (
            mock.mock_open(read_data="mytemplate").return_value,
    ...
    @mock.patch(
        "tethys_apps.template_loaders.io.open", side_effect=IOError(errno.ENOENT, "foo")
    )
    @mock.patch("tethys_apps.template_loaders.BaseLoader")
    def test_get_contents_template_does_not_exist(self, _, mock_file):
        ...
        self.assertRaises(
            TemplateDoesNotExist, tethys_template_loader.get_contents, origin
        )
        mock_file.assert_called_once()
