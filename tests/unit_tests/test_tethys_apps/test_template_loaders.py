...
class TestTethysTemplateLoader(unittest.TestCase):
    ...
    def test_get_contents(self, mock_baseloader, mock_file):
        ...
        tethys_template_loader = TethysTemplateLoader(mock_baseloader)
    ...
    @mock.patch('tethys_apps.template_loaders.BaseLoader')
    def test_get_contents_io_error(self, mock_baseloader, mock_file):
        mock_file.side_effect = IOError
        origin = mock.MagicMock(name='test_app/css/main.css')

        tethys_template_loader = TethysTemplateLoader(mock_baseloader)

        self.assertRaises(Exception, tethys_template_loader.get_contents, origin)
        mock_file.assert_called_once()

    @mock.patch('tethys_apps.template_loaders.io.open', side_effect=IOError(errno.ENOENT, 'foo'))
    @mock.patch('tethys_apps.template_loaders.BaseLoader')
    def test_get_contents_template_does_not_exist(self, mock_baseloader, mock_file):
        origin = mock.MagicMock(name='test_app/css/main.css')
    ...
    def test_get_template_sources(self, mock_gdt, mock_bl):
        tethys_template_loader = TethysTemplateLoader(mock_bl)
    ...
    @mock.patch('tethys_apps.template_loaders.BaseLoader')
    @mock.patch('tethys_apps.template_loaders.get_directories_in_tethys')
    def test_get_template_sources_exception(self, mock_gdt, mock_bl, mock_safe_join):
