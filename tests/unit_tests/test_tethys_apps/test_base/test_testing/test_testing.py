...
class TestTethysTestCase(unittest.TestCase):
    ...
    @mock.patch(
        "tethys_apps.base.testing.testing.TethysAppBase.create_persistent_store"
    )
    ...
    def test_create_test_persistent_stores_for_app(self, mock_ite, mock_ta, mock_cps):
        ...
        mock_cps.assert_called_with(
            connection_name=None,
            db_name="test_name",
            force_first_time=True,
            initializer="test_init",
            refresh=True,
    ...
    def test_create_test_persistent_stores_for_app_not_testing_env(self, mock_ite):
        ...
        self.assertRaises(
            EnvironmentError,
            base_testing.TethysTestCase.create_test_persistent_stores_for_app,
            app_class,
        )
    ...
    @mock.patch("tethys_apps.models.TethysApp")
    @mock.patch("tethys_apps.base.testing.testing.is_testing_environment")
    def test_create_test_persistent_stores_for_app_not_success(
        self, mock_ite, mock_ta, mock_cps
    ):
        mock_ite.return_value = True
        app_class = TethysAppBase
    ...
    @mock.patch(
        "tethys_apps.base.testing.testing.TethysAppBase.list_persistent_store_databases"
    )
    @mock.patch("tethys_apps.base.testing.testing.is_testing_environment")
    def test_destroy_test_persistent_stores_for_app(
        self, mock_ite, mock_lps, mock_get, mock_dps
    ):
        mock_ite.return_value = True
        app_class = TethysAppBase
    ...
    def test_destroy_test_persistent_stores_for_app_not_testing_env(self, mock_ite):
        ...
        self.assertRaises(
            EnvironmentError,
            base_testing.TethysTestCase.destroy_test_persistent_stores_for_app,
            app_class,
        )
    ...
    def test_create_test_user(self, mock_user):
        ...
        result = base_testing.TethysTestCase.create_test_user(
            username="test_user", password="test_pass", email="test_e"
        )
    ...
    def test_create_test_super_user(self, mock_user):
        mock_user.objects.create_superuser = mock.MagicMock(
            return_value="test_create_super_user"
        )

        result = base_testing.TethysTestCase.create_test_superuser(
            username="test_user", password="test_pass", email="test_e"
        )

        # Check result
        self.assertEqual("test_create_super_user", result)
        mock_user.objects.create_superuser.assert_called_with(
            username="test_user", password="test_pass", email="test_e"
        )
