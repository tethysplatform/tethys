...
class TestTethysPath(unittest.TestCase):
    ...
    def test_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(Path(os.path.join(temp_dir, 'file1.txt')) in tethys_path_files)
            self.assertTrue(Path(os.path.join(temp_dir, 'file2.txt')) in tethys_path_files)
            self.assertTrue(Path(os.path.join(temp_dir, 'file3.txt')) in tethys_path_files)
            self.assertEqual(len(tethys_path_files), 3)
    ...
    def test_directories(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(Path(os.path.join(temp_dir, 'dir1')) in tethys_path_directories)
            self.assertTrue(Path(os.path.join(temp_dir, 'dir2')) in tethys_path_directories)
            self.assertTrue(Path(os.path.join(temp_dir, 'dir3')) in tethys_path_directories)
            self.assertEqual(len(tethys_path_directories), 3)
...
class TestTethysPathHelpers(unittest.TestCase):
    ...
    def test_get_app_workspace_old(self, mock_get_active_app, mock_get_app_workspace_old):
        mock_get_active_app.return_value = self.mock_app
    ...
    @mock.patch('tethys_apps.utilities.get_active_app')
    @override_settings(USE_OLD_WORKSPACES_API=True)
    def test_get_user_workspace_old(self, mock_get_active_app, mock_get_user_workspace_old):
        mock_get_active_app.return_value = self.mock_app
        mock_return = mock.MagicMock()
