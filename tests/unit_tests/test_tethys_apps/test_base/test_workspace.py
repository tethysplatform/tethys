...
class TestUrlMap(unittest.TestCase):
    ...
    def test_user_workspace_user(self, mock_guw, _, mock_rq, mock_log):
        ...
        mock_request = mock.MagicMock(spec=AsgiRequest, user=mock.MagicMock(spec=User))
    ...
    def test_user_workspace_no_ASGIRequest(self):
    ...
    def test_app_workspace_no_ASGIRequest(self):
        mock_request = mock.MagicMock()
        ret = None
        with self.assertRaises(ValueError) as context:
            ret = app_dec_controller(mock_request)
