...
class TestTethysAsyncWebsocketConsumer(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.consumer = tethys_app_base.TethysAsyncWebsocketConsumer()
    ...
    async def test_on_authorized_connect(self):
        await self.consumer.on_authorized_connect()

    async def test_on_connect(self):
        await self.consumer.on_connect()

    async def test_on_disconnect(self):
        event = {}
        await self.consumer.on_disconnect(event)

    async def test_on_receive(self):
        event = {}
    ...
    async def test_connect(self, mock_accept, mock_on_connect, mock_on_authorized_connect):
        ...
        mock_on_authorized_connect.assert_called_once()
    ...
    async def test_connect_not_authorized(self, mock_accept, mock_on_connect, mock_close):
        ...
        mock_accept.assert_called_once()
        mock_on_connect.assert_called_once()
        mock_close.assert_called_with(code=4004)

    @mock.patch("tethys_apps.base.app_base.TethysAsyncWebsocketConsumer.on_disconnect")
    async def test_disconnect(self, mock_on_disconnect):
        event = "event"
        await self.consumer.disconnect(event)
        mock_on_disconnect.assert_called_with(event)
