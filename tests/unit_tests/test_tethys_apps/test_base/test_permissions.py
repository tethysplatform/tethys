...
class TestPermission(unittest.TestCase):
    def setUp(self):
        ...
        self.check_string = '<Permission name="{0}" description="{1}">'.format(
...
class TestPermissionGroup(unittest.TestCase):
    ...
    def test_init(self):
        result = tethys_permission.PermissionGroup(
            name=self.name, permissions=["foo", "bar"]
        )
...
class TestAsyncPermissionGroup(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_scoped_user_has_permission(self, mock_app):
        ...
        mock_app.return_value = mock.MagicMock(package="test_package")
        result = await tethys_permission.scoped_user_has_permission(scope=scope, perm="test_perm")
        self.assertTrue(result)
