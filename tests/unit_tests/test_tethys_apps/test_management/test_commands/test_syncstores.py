...
class ManagementCommandsSyncstoresTests(unittest.TestCase):
    ...
    @mock.patch(
        "tethys_apps.management.commands.syncstores.Command.provision_persistent_stores"
    )
    ...
    def test_provision_persistent_stores_all_apps_no_database(
        self, mock_app, mock_setting1, mock_setting2, mock_setting3, mock_stdout
    ):
        # Mock arguments
        ...
        mock_setting1.create_persistent_store_database.assert_called_once_with(
            refresh=True, force_first_time=True
        )
        mock_setting2.create_persistent_store_database.assert_called_once_with(
            refresh=True, force_first_time=True
        )
        mock_setting3.create_persistent_store_database.assert_called_once_with(
            refresh=True, force_first_time=True
        )
    ...
    @mock.patch("tethys_apps.models.TethysApp.persistent_store_database_settings")
    @mock.patch("tethys_apps.models.TethysApp")
    def test_provision_persistent_stores_all_apps_database_no_match(
        self, mock_app, mock_setting1, mock_setting2, mock_setting3, mock_stdout
    ):
        # Mock arguments
        mock_app_names = syncstores.ALL_APPS
        mock_options = {
            "database": "/foo/no_match",
            "refresh": True,
            "first_time": True,
        }
    ...
    @mock.patch("tethys_apps.models.TethysApp.persistent_store_database_settings")
    @mock.patch("tethys_apps.models.TethysApp")
    def test_provision_persistent_stores_all_apps_database_single_match(
        self, mock_app, mock_setting1, mock_setting2, mock_setting3, mock_stdout
    ):
        # Mock arguments
        mock_app_names = syncstores.ALL_APPS
    ...
    @mock.patch("sys.stdout", new_callable=StringIO)
    @mock.patch("tethys_apps.models.TethysApp")
    def test_provision_persistent_stores_given_apps_not_found(
        self, mock_app, mock_stdout
    ):
        # Mock arguments
        ...
        self.assertIn(
            'The app named "foo_missing" cannot be found.', mock_stdout.getvalue()
        )
        self.assertIn(
            "Please make sure it is installed and try again.", mock_stdout.getvalue()
        )
        self.assertIn("Provisioning Persistent Stores...", mock_stdout.getvalue())
