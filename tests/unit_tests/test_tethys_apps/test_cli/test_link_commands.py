...
class TestLinkCommands(unittest.TestCase):
    ...
    @mock.patch('tethys_apps.cli.link_commands.exit')
    @mock.patch('tethys_apps.cli.link_commands.link_service_to_app_setting')
    @mock.patch('tethys_apps.cli.link_commands.pretty_output')
    def test_link_commands(self, _, mock_link_app_setting, mock_exit):
        args = mock.MagicMock(service='persistent_connection:super_conn', setting='epanet:database:epanet_2')
    ...
    @mock.patch('tethys_apps.cli.link_commands.link_service_to_app_setting')
    @mock.patch('tethys_apps.cli.link_commands.pretty_output')
    def test_link_commands_index_error(self, mock_pretty_output, mock_link_app_setting, mock_exit):
    ...
    @mock.patch('tethys_apps.cli.link_commands.link_service_to_app_setting')
    @mock.patch('tethys_apps.cli.link_commands.pretty_output')
    def test_link_commands_with_exception(self, mock_pretty_output, mock_link_app_setting, mock_exit):
        # NOTE: We have the mocked exit function raise a SystemExit exception to break the code
        # execution like the original exit function would have done.
        ...
        po_call_args = mock_pretty_output().__enter__().write.call_args_list
        self.assertIn("An unexpected error occurred. Please try again.", po_call_args[1][0][0])
        mock_exit.assert_called_with(1)

    @mock.patch('tethys_apps.cli.link_commands.exit')
    @mock.patch('tethys_apps.cli.link_commands.link_service_to_app_setting', return_value=False)
    @mock.patch('tethys_apps.cli.link_commands.pretty_output')
