...
class TestServiceInstallHelpers(TestCase):
    ...
    @mock.patch(
        "tethys_services.models.PersistentStoreService.objects.get", return_value=True
    )
    def test_get_service_from_id_persistent(self, mock_get):
        self.assertEqual(
            install_commands.get_service_from_id(1).get("service_type"), "persistent"
        )
        mock_get.assert_called_with(id=1)

    @mock.patch(
        "tethys_services.models.SpatialDatasetService.objects.get", return_value=True
    )
    def test_get_service_from_id_spatial(self, mock_get):
        self.assertEqual(
            install_commands.get_service_from_id(1).get("service_type"), "spatial"
        )
        mock_get.assert_called_with(id=1)
    ...
    def test_get_service_from_id_dataset(self, mock_get):
        self.assertEqual(
            install_commands.get_service_from_id(1).get("service_type"), "dataset"
        )
        mock_get.assert_called_with(id=1)

    @mock.patch(
        "tethys_services.models.WebProcessingService.objects.get", return_value=True
    )
    def test_get_service_from_id_wps(self, mock_get):
        self.assertEqual(
            install_commands.get_service_from_id(1).get("service_type"), "wps"
        )
        mock_get.assert_called_with(id=1)
    ...
    def test_get_service_from_name_persistent(self, mock_get):
        self.assertEqual(
            install_commands.get_service_from_name("nonexisting").get("service_type"),
            "persistent",
        )
    ...
    @mock.patch(
        "tethys_apps.cli.install_commands.get_service_from_name",
        return_value={"service_type": "service_type", "linkParam": "linkParam"},
    )
    ...
    def test_find_and_link(self, mock_link_service_to_app_setting, _):
        ...
        install_commands.find_and_link(
            service_type, setting_name, service_name, app_name
        )

        mock_link_service_to_app_setting.assert_called_with(
            "service_type", service_name, app_name, "linkParam", setting_name
        )

    @mock.patch(
        "tethys_apps.cli.install_commands.get_service_from_name", return_value=False
    )
    @mock.patch("tethys_apps.cli.cli_colors.pretty_output")
    def test_find_and_link_warning(self, mock_pretty_output, _):
        ...
        po_call_args = mock_pretty_output().__enter__().write.call_args_list
        self.assertEqual(
            f"Warning: Could not find service of type: {service_type} with the name/id: {service_name}",
            po_call_args[0][0][0],
        )
...
class TestInstallServicesCommands(TestCase):
    ...
    def test_configure_services(
        ...
        services = {
            "version": 1,
            "custom_setting": {custom_setting_name: custom_service_name},
        ...
        po_call_args = mock_pretty_output().__enter__().write.call_args_list
        self.assertIn(
            f"Incorrect value type given for custom setting '{custom_setting_name}'",
            po_call_args[0][0][0],
        )

        mock_find_and_link.assert_called_with(
            "persistent", persistent_setting_name, persistent_service_name, app_name
        )
    ...
    def test_run_portal_install(
        self, mock_os, mock_open_file, mock_pretty_output, mock_configure_services
    ):
        app_name = "foo"
...
class TestInstallCommands(TestCase):
    ...
    @mock.patch("tethys_apps.cli.install_commands.call")
    @mock.patch(
        "tethys_apps.cli.install_commands.run_portal_install", return_value=False
    )
    @mock.patch("tethys_apps.cli.install_commands.run_services")
    @mock.patch("tethys_apps.cli.install_commands.exit")
    @mock.patch("tethys_apps.cli.cli_colors.pretty_output")
    def test_skip_input_file(
        self, mock_pretty_output, mock_exit, _, __, ___, ____, _____, ______
    ):
        ...
        po_call_args = mock_pretty_output().__enter__().write.call_args_list
        self.assertEqual(
            "Skipping package installation, Skip option found.", po_call_args[0][0][0]
        )
        mock_exit.assert_called_with(0)
    ...
    def test_conda_and_pip_package_install(
        self, mock_pretty_output, mock_exit, mock_conda_run, mock_call, _
    ):
        file_path = os.path.join(self.root_app_path, "install-dep.yml")
        args = mock.MagicMock(
            file=file_path, develop=False, verbose=False, services_file=None
        )
        mock_exit.side_effect = SystemExit
        ...
        self.assertIn(
            "Warning: Packages installation ran into an error.", po_call_args[1][0][0]
        )
    ...
    def test_interactive_custom_setting_set(
        self, mock_pretty_output, mock_get_settings, _
    ):
    ...
    def test_interactive_custom_setting_interrupt(
        self, mock_pretty_output, mock_get_settings, mock_exit, _
    ):
        mock_cs = mock.MagicMock()
        mock_cs.name = "mock_cs"
        mock_get_settings.return_value = {"unlinked_settings": [mock_cs]}
    ...
    @mock.patch(
        "tethys_apps.cli.install_commands.get_setting_type", return_value="persistent"
    )
    @mock.patch(
        "tethys_apps.cli.install_commands.get_service_from_id", side_effect=ValueError
    )
    @mock.patch(
        "tethys_apps.cli.install_commands.get_service_from_name",
        side_effect=[False, {"service_type": "st", "linkParam": "lp"}],
    )
    @mock.patch("tethys_apps.cli.install_commands.exit")
