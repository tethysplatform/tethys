...
class TethysPortalViewsAccountsTest(unittest.TestCase):
    ...
    def test_auth_is_mtm_multi_tenant_none(self, mock_do_auth):
        ...
        mock_backend.setting = mock.MagicMock(
            return_value=None
        )  # MULTI_TENANT returns None
        mock_request = mock.MagicMock(method="GET", backend=mock_backend)  # GET request
        url_backend = "foo"
    ...
    @mock.patch("tethys_portal.views.psa.redirect")
    @mock.patch("tethys_portal.views.psa.do_complete")
    def test_complete_is_mtm_no_saved_tenant(
        self, mock_do_complete, mock_redirect, mock_log
    ):
        mock_backend = mock.MagicMock(
            spec=MultiTenantMixin,
            strategy=mock.MagicMock(session_get=mock.MagicMock(return_value=None)),
        )
        mock_backend.name = "foo"
        mock_request = mock.MagicMock(method="GET", backend=mock_backend)  # GET request
        url_backend = "foo"
    ...
    def test_complete_is_mtm_tenant_valid(self, mock_do_complete):
        mock_backend = mock.MagicMock(
            spec=MultiTenantMixin,
            strategy=mock.MagicMock(session_get=mock.MagicMock(return_value="Foo")),
        )
        mock_backend.name = "foo"
        mock_request = mock.MagicMock(method="GET", backend=mock_backend)  # GET request
        url_backend = "foo"
    ...
    @mock.patch("tethys_portal.views.psa.redirect")
    @mock.patch("tethys_portal.views.psa.do_complete")
    def test_complete_is_mtm_improperly_configured(
        self, mock_do_complete, mock_redirect, mock_log
    ):
        mock_backend = mock.MagicMock(
            spec=MultiTenantMixin,
        ...
        type(mock_backend).tenant = mock.PropertyMock(
            side_effect=ImproperlyConfigured("some error")
        )
        mock_backend.name = "foo"
        mock_request = mock.MagicMock(method="GET", backend=mock_backend)  # GET request
        url_backend = "foo"
    ...
    @mock.patch("tethys_portal.views.psa.redirect")
    @mock.patch("tethys_portal.views.psa.do_complete")
    def test_complete_is_mtm_value_error(
        self, mock_do_complete, mock_redirect, mock_log
    ):
        mock_backend = mock.MagicMock(
            spec=MultiTenantMixin,
    ...
    def test_tenant_get_backend_not_mtm(self, mock_redirect, mock_log):
        ...
        mock_request = mock.MagicMock(
            method="GET", GET=dict(), backend=mock_backend
        )  # GET request
        url_backend = "foo"
    ...
    def test_tenant_view_post_no_submit(self, mock_tenant_form):
        mock_backend = mock.MagicMock(spec=MultiTenantMixin)
        mock_request = mock.MagicMock(
            method="POST", POST=dict(), backend=mock_backend
        )  # Empty POST dict
        url_backend = "foo"
    ...
    def test_tenant_view_post_valid(self, mock_tenant_form, mock_do_auth):
        ...
        mock_request = mock.MagicMock(
            method="POST", POST=post_params, backend=mock_backend
        )  # valid POST request
        url_backend = "foo"
    ...
    @mock.patch("tethys_portal.views.psa.do_auth")
    @mock.patch("tethys_portal.views.psa.SsoTenantForm", spec=SsoTenantForm)
    def test_tenant_view_post_improperly_configured(
        self, mock_tenant_form, mock_do_auth, mock_redirect, mock_log
    ):
        mock_tenant_form.is_valid = mock.MagicMock(return_value=True)
        mock_tenant_form().cleaned_data = {"tenant": "GitHub"}
        mock_backend = mock.MagicMock(spec=MultiTenantMixin)
    ...
    @mock.patch("tethys_portal.views.psa.do_auth")
    @mock.patch("tethys_portal.views.psa.SsoTenantForm", spec=SsoTenantForm)
    def test_tenant_view_post_value_error(
        self, mock_tenant_form, mock_do_auth, mock_render
    ):
        mock_tenant_form.is_valid = mock.MagicMock(return_value=True)
        mock_tenant_form().cleaned_data = {"tenant": "GitHub"}
        mock_backend = mock.MagicMock(spec=MultiTenantMixin)
