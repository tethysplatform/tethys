...
@test.override_settings(
    OAUTH_CONFIG={"SOCIAL_AUTH_ARCGIS_PORTAL_URL": "https://my.test.arcgis/portal"}
)
class ArcGISPortalBackendTest(test.SimpleTestCase):
    ...
    def test_attributes(self):
        ...
        self.assertEqual(
            f"{inst.PORTAL_URL}/sharing/rest/oauth2/authorize", inst.AUTHORIZATION_URL
        )
        self.assertEqual(
            f"{inst.PORTAL_URL}/sharing/rest/oauth2/token", inst.ACCESS_TOKEN_URL
        )

    @test.override_settings(OAUTH_CONFIG={"SOCIAL_AUTH_ARCGIS_PORTAL_URL": ""})
    def test_oidc_endpoint__no_portal_url(self):
        with self.assertRaises(ValueError) as context:
            inst = ArcGISPortalOAuth2(self.strategy)
            inst.PORTAL_URL
        self.assertEqual(
            "You must specify the url of your ArcGIS Enterprise Portal via "
    ...
    def test_user_data(self):
        ...
        inst.get_json.assert_called_once_with(
            ...
            params={"token": "my_special_token_12345", "f": "json"},
        )
