...
class OneLoginOIDCBackendTest(test.SimpleTestCase):
    ...
    def generate_access_token(self):
        ...
        access_token = jwt.encode(
            access_claims, self.key_pair, algorithm="RS256", headers={"kid": self.kid}
        )
        return access_token
    ...
    def generate_id_token(self, access_token, at_hash=None):
        ...
        id_token = jwt.encode(
            id_claims, self.key_pair, algorithm="RS256", headers={"kid": self.kid}
        )
        return id_token
    ...
    def test_validate_and_return_id_token__no_valid_key(self):
        ...
        self.assertEqual(
            str(cm.exception), "Token error: Signature verification failed"
        )
    ...
    def test_validate_and_return_id_token__claims_error(self):
        ...
        self.assertEqual(
            str(cm.exception), "Token error: at_hash claim does not match access_token."
        )
