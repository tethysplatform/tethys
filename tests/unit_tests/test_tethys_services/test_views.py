...
class TethysServicesViewsTests(unittest.TestCase):
    ...
    def test_datasets_home(self, mock_render):
        ...
        self.assertEquals('datasets_home', datasets_home(mock_request))
        mock_render.assert_called_once_with(mock_request, 'tethys_services/tethys_datasets/home.html', context)
    ...
    def test_wps_home(self, mock_render, mock_list_wps_service_engines):
        ...
        self.assertEquals('wps_home', wps_home(mock_request))
        mock_render.assert_called_once_with(mock_request, 'tethys_services/tethys_wps/home.html', context)
    ...
    def test_wps_service(self, mock_render, mock_get_wps_service_engine):
        ...
        self.assertEquals('wps_service', wps_service(mock_request, mock_service))
        mock_render.assert_called_once_with(mock_request, 'tethys_services/tethys_wps/service.html', context)
        mock_get_wps_service_engine.assert_called_once_with(mock_service)

    @mock.patch('tethys_services.views.abstract_is_link')
    @mock.patch('tethys_services.views.get_wps_service_engine')
    @mock.patch('tethys_services.views.render')
    def test_wps_process(self, mock_render, mock_get_wps_service_engine, mock_abstract_is_link):
        mock_request = mock.MagicMock()
        mock_service = mock.MagicMock()
        ...
        self.assertEquals('wps_process', wps_process(mock_request, mock_service, mock_identifier))
