...
class TestCommandTests(unittest.TestCase):
    ...
    def test_db_command_create(self, mock_create_db_user):
        ...
        self.assertEqual(call_args[0], mock.call(port=self.options['port'], username=self.options['username'],
                                                 password=self.options['password'], db_name=self.options['db_name']))
        mock_create_db_user.assert_called_with(port=self.options['port'], username=self.options['superuser_name'],
    ...
    def test_db_command_create_db_user(self):
        ...
        self.assertEqual(call_args[0][0][0], ['psql', '-U', 'postgres', '-p', '0000', '--command', command])
        args = ['createdb', '-U', 'postgres', '-p', self.options['port'], '-O', self.options['username'],
    ...
    @mock.patch('tethys_cli.db_commands.django.setup')
    def test_db_command_createsuperuser(self, mock_setup, mock_create_superuser):
        mock_args = mock.MagicMock()
