...
class TestDatatableView(unittest.TestCase):
    ...
    def test_DataTableView(self):
        ...
        rows = 2
        result = gizmo_datatable_view.DataTableView(
            rows=rows, column_names=column_names, datatable_options=datatable_options
        )
        # Check Result
        self.assertEqual(rows, result["rows"])
        self.assertEqual(column_names, result["column_names"])
        self.assertIn(
            datatable_options["rows"][0][0],
            result["datatable_options"]["datatable_options"],
        )
        self.assertIn(
            datatable_options["rows"][0][1],
            result["datatable_options"]["datatable_options"],
        )
        self.assertIn(
            datatable_options["rows"][0][2],
            result["datatable_options"]["datatable_options"],
        )
        self.assertIn(
            datatable_options["rows"][1][0],
            result["datatable_options"]["datatable_options"],
        )
        self.assertIn(
            datatable_options["rows"][1][1],
            result["datatable_options"]["datatable_options"],
        )
        self.assertIn(
            datatable_options["rows"][1][2],
            result["datatable_options"]["datatable_options"],
        )
