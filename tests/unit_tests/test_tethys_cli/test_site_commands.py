...
from tethys_cli.site_commands import gen_site_content, update_site_settings_content, uncodify
...
class CLISiteCommandsTest(unittest.TestCase):
    ...
    def test_gen_site_content(self, mock_load_apps, mock_update_settings):
        ...
        mock_args = mock.MagicMock(brand_text="New Title", restore_defaults=False, from_file=False)
    ...
    @mock.patch('tethys_cli.site_commands.load_apps')
    def test_gen_site_content_with_yaml(self, mock_load_apps, mock_path, mock_update_settings):
    ...
    @mock.patch('tethys_cli.site_commands.Path')
    @mock.patch('tethys_cli.site_commands.load_apps')
    def test_gen_site_content_with_yaml_invalid_category(self, mock_load_apps, mock_path, mock_update_settings,
                                                         mock_load_yaml, mock_warn):
        mock_file_path = mock.MagicMock()
        mock_path.return_value = mock_file_path
        mock_file_path.__truediv__().exists.return_value = True
    ...
    @mock.patch('tethys_cli.site_commands.Path')
    def test_gen_site_content_with_create_yaml(self, mock_path, mock_call, mock_exit, mock_pretty_output, _, __):
    ...
    def test_update_site_settings_content_invalid_setting(self, mock_filter, mock_warn):
        ...
        update_site_settings_content({'invalid_setting': 'content'}, warn_if_setting_not_found=True)
