...
class CLISiteCommandsTest(unittest.TestCase):
    ...
    def test_gen_site_content(self, mock_load_apps, mock_now, mock_filter):
        ...
        mock_args = mock.MagicMock(title="New Title", restore_defaults=False, file=False)
    ...
    def test_gen_site_content_restore_defaults(self, mock_load_apps, mock_all, mock_get, mock_setting_defaults):
        mock_args = mock.MagicMock(file=False)
    ...
    @mock.patch('tethys_cli.site_commands.timezone.now')
    @mock.patch('tethys_cli.site_commands.load_apps')
    def test_gen_site_content_with_yaml(self, mock_load_apps, mock_now, mock_filter):
        file_path = os.path.join(self.root_app_path, 'test-site-content.yml')

        mock_args = mock.MagicMock(file=file_path, restore_defaults=False)
    ...
    @mock.patch('tethys_cli.site_commands.os')
    def test_gen_site_content_with_create_yaml(self, mock_os, mock_call, mock_exit, mock_pretty_output, _):
        mock_args = mock.MagicMock(restore_defaults=False)
        mock_os.path.exists.return_value = False

        gen_site_content(mock_args)
        po_call_args = mock_pretty_output().__enter__().write.call_args_list
        self.assertIn('Generation of site_content file cancelled', po_call_args[0][0][0])

        gen_site_content(mock_args)
        self.assertIn('site_content', mock_call.call_args_list[0][0][0])
