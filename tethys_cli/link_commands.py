...
def add_link_parser(subparsers):
    ...
    link_parser = subparsers.add_parser(
        "link",
        help="Link a Service to a Tethys app Setting.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
formats:
  service    <service_type>:<service_id|service_name>
  setting    <app_package>:<setting_type>:<setting_id|setting_name>

service types:
  condor, dask, dataset, persistent, spatial, wps, secure_map

setting types:
  ps_database, ps_connection, ds_spatial, ds_dataset, ss_scheduler, wps,
  secure_map

examples:
  tethys link persistent:super_conn epanet:ps_database:epanet_2
""",
    )

    # tethys link
    link_parser.add_argument("service", help="Service to link to a target app.")
    link_parser.add_argument(
        "setting", help="Setting of an app with which to link the specified service."
    )
        ...
        try:
            ...
            with pretty_output(FG_RED) as p:
                p.write(
                    ...
                    '<app_package>:<setting_type (ps_database|ps_connection|ds_spatial|ds_dataset|ss_scheduler|wps|secure_map)><setting_id|setting_name>"'
                    "\nCommand aborted."
                )
            exit(1)
