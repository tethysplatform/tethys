...
if [ "$(uname)" = "Linux" ] then
    # LINUX_DISTRIBUTION=$(lsb_release -is) || LINUX_DISTRIBUTION=$(python -c "import platform; print(platform.linux_distribution(full_distribution_name=0)[0])") convert to lower case LINUX_DISTRIBUTION=${LINUX_DISTRIBUTION,,}
...
elif [ "$(uname)" = "Darwin" ] # i.e. MacOSX then
    ...
    exit fi
# Set default options
ALLOWED_HOST='127.0.0.1' TETHYS_HOME=~/tethys TETHYS_PORT=8000 TETHYS_DB_USERNAME='tethys_default' TETHYS_DB_PASSWORD='pass' TETHYS_DB_HOST='127.0.0.1' TETHYS_DB_PORT=5436 CONDA_ENV_NAME='tethys' PYTHON_VERSION='2' 
BRANCH='release' TETHYS_SUPER_USER='admin' TETHYS_SUPER_USER_EMAIL='' TETHYS_SUPER_USER_PASS='pass' DOCKER_OPTIONS='-d'
...
while [[ $# -gt 0 ]] do key="$1" case $key in
    ...
    *) # unknown option
    ...
    ;; esac shift # past argument or value done
# resolve relative paths
resolve_relative_path TETHYS_HOME ${TETHYS_HOME}
# set CONDA_HOME relative to TETHYS_HOME if not already set
if [ -z ${CONDA_HOME} ] then
    CONDA_HOME="${TETHYS_HOME}/miniconda" else
    resolve_relative_path CONDA_HOME ${CONDA_HOME} fi if [ -n "${ECHO_COMMANDS}" ] then
    set -x # echo commands as they are executed fi
# Set paths for environment activate/deactivate scripts
ACTIVATE_DIR="${CONDA_HOME}/envs/${CONDA_ENV_NAME}/etc/conda/activate.d" DEACTIVATE_DIR="${CONDA_HOME}/envs/${CONDA_ENV_NAME}/etc/conda/deactivate.d" ACTIVATE_SCRIPT="${ACTIVATE_DIR}/tethys-activate.sh" 
DEACTIVATE_SCRIPT="${DEACTIVATE_DIR}/tethys-deactivate.sh" if [ -z "${SKIP_TETHYS_INSTALL}" ] then
    ...
    # initdb -U postgres -D "${TETHYS_HOME}/psql/data" pg_ctl -U postgres -D "${TETHYS_HOME}/psql/data" -l "${TETHYS_HOME}/psql/logfile" start -o "-p ${TETHYS_DB_PORT}"
    ...
    echo "alias t='. ${CONDA_HOME}/bin/activate ${CONDA_ENV_NAME}'" >> ~/${BASH_PROFILE} fi
...
TETHYS_CONDA_HOME=${CONDA_HOME} TETHYS_CONDA_ENV_NAME=${CONDA_ENV_NAME}
...
${TETHYS_HOME}/src/tethys_portal/tethys-selinux-policy.te
...
if [ -n "${LINUX_DISTRIBUTION}" -a "${PRODUCTION}" = "true" ] then
    # prompt for sudo
    ...
    echo "unalias trs" >> "${DEACTIVATE_SCRIPT}" fi
# Install Docker (if flag is set
...
if [ -n "${LINUX_DISTRIBUTION}" -a "${INSTALL_DOCKER}" = "true" ] then
    # prompt for sudo
    ...
    case ${LINUX_DISTRIBUTION} in
        ...
        *)
        ...
        ;;
    esac fi if [ -z "${SKIP_TETHYS_INSTALL}" ] then
    echo "Tethys installation complete!"
    echo
    echo "NOTE: to enable the new alias 't' which activates the tethys environment you must run '. ~/${BASH_PROFILE}'" fi on_exit(){
    set +e
    set +x
}
trap on_exit EXIT
